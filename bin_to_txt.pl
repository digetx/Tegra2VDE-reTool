...
sub write_record {
    ...
    if ($type == 8) {
        printf TXTFILE ("%s: %s 0x%08X 0x%08X\t\"$dsc\" %d\n",
                        $src, $defines[$type], $val1, $val2, $val3);
    } else {
        printf TXTFILE ("%s: %s 0x%08X 0x%08X\t\"$dsc\"\n",
                        $src, $defines[$type], $val1, $val2);
    }
}
